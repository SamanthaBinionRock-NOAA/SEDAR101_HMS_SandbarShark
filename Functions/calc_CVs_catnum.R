...
CVs.catnum.MRIP = function( rdi.report, inc.modes, genrec.table = NA,
                            imp.1981w1 = 'None',
  ...
  if( !Carib.SEDAR ) {
    ...
    cols.extract <- c( "t.YEAR", paste0( "t.",toupper(inc.modes),"_AB1" ) )
    ...
    if( length(rdi.report) == 1 ) {
      ...
      mrip.mode = dbGetQuery(con,
                             paste0("select ", cols.extract, " ",
                                ...
                                where t.APP_USER = ", sprintf("'%s'", paste( report.name, collapse = "','" ))
                             ))
      ...
      colnames( mrip.total ) <- c( "YEAR", "TOTAL_AB1", "TOTAL_CV_AB1", "TOTAL_B2", "TOTAL_CV_B2",
                                   "TOTAL_PSU_AB1", "TOTAL_PSU_B2", "TOTAL_PSU_Tot", "TOTAL_AT_AB1", "TOTAL_AT_B2", "TOTAL_AT_Tot" )
      ###       Note that I invert the 'meaning' of the c( "SZ","N" ) cols because they are defined differently b/w "mrip.mode" & "mrip.total"...
    ...
    rm( cols.extract, mrip.mode,mrip.total )
    ...
    if( flag.forhire ) {
      ...
      ###     For the two 'ratio' components in the above equation (i.e., catch & variance estimates for CBT:HBT ratios), these are
      ###     saved in an excel spreadsheet constructed for SEDAR 82 (SATL GTF) that was based on calculations of the number of angler-trips
      ###     from raw intercept data ( TRP = CNTRBTRS * WP_INT ) for MATL/NATL CBT & HBT vessels btw 1981-2003...
      ...
      ###     For the two 'forhire' components in the above equation (i.e., catch & variance estimates of the combined for-hire mode)...
      ###
      ###       -- Catch estimates are imported from the 'catch.table.forhire', which is a filtered copy of the 'catch.table' object
      ###         ( to only include MATL/NATL forhire catch estimates ) created in the main script right before any 'CBT_HBT' estimates
      ###         were partitioned amongst modes...
      ###
      ###       -- However, the associated variance estimates are a bit trickier... While CVs could be imported from RDI,
      ###         the default CV report provides estimates at either the year-mode or year-state level, not at the
      ###         year-mode-state level needed for this calculation (to match the resolution at which the forhire allocation ratios,
      ###         in our 'ratios' object, are applied). Instead, the approach below assumes the (year-mode) forhire CVs in the
      ###         current 'cv.table' are representative of the associated year-mode-state CVs, wherein the CVs for all states
      ###         (within a given year-mode combination) will be assigned the same uncertainty estimate...
      ...
      dummy = catch.table.forhire %>%
        ...
        ###     ...where the strata-specific (CBTHBT) estimates in 'catch.table.forhire' are summed to provide total estimates
        ###         by year and state, which is the resolution at which CBTHBT estimates are partitioned (into separate CBT & HBT modes)...
        group_by( YEAR, NEW_STA ) %>%
        summarise( AB1 = sum( AB1, na.rm=TRUE ),
                    B2 = sum(  B2, na.rm=TRUE ) ) %>%
        ungroup() %>%
        
        ###     ...join these (combined forhire) estimates with the associated allocation ratios (and their variances)...
        left_join( ratios, by = c('YEAR','NEW_STA') )
        
        ###     ...and finally join this table with the combined forhire elements contained in 'cv.table',
        ###     wherein these year-mode uncertainty estimates will be used as a proxy for the uncertainties of
        ###     all associated year-mode-state estimates (i.e.,the CVs for all states within a given
        ###     year-mode combination will be assigned the same uncertainty estimate)...
      ...
      dummy = dummy %>%
        ...
        select( YEAR, NEW_STA, AB1, AB1_var, B2, B2_var, Hbt_ratio_est, Hbt_ratio_var, Cbt_ratio_est, Cbt_ratio_var ) %>%
        ...
        select( YEAR, NEW_STA, CBT_AB1, CBT_VAR_AB1, HBT_AB1, HBT_VAR_AB1, CBT_B2, CBT_VAR_B2, HBT_B2, HBT_VAR_B2 ) %>%
        
        ###     Lastly, I aggregate these year-mode-state estimates (summed across states) into the final
        ###     (year-mode) resolution, which is the resolution at which CVs estimates are provided ( in 'cv.table' )...
        group_by( YEAR ) %>%
        ...
        # select( YEAR, CBT_AB1, CBT_CV_AB1, HBT_AB1, HBT_CV_AB1, CBT_B2, CBT_CV_B2, HBT_B2, HBT_CV_B2 )
      ...
      blah = blah %>%
        ...
        ###   As a last step, I need to recalculate the CV columns for Cbt & Hbt, from the newly summed
        ###         catch and variance fields for these modes...
        ...
        ###         ...after which, I no longer need these variance fields, and so drop them...
    ...
    if( imp.1981w1 != 'None' ) {
      ...
      ###   In addition to the MRIP-1981 filter above, there are two other filters that we (might) need to apply
      ###   before we can pull the appropriate (1981-MRIP) catch estimates from 'genrec.table':
      ###
      ...
      if( length(rdi.report) == 1 ) {
        ...
        ### -----------------------------------
        ###
        ###     ...for which no additional (spatial) filter needs to be applied to 'genrec.table'...
    ...
    if( length(rdi.report) == 1 ) {
      ...
      cv.table <- dummy.cv
      rm( col.IDs, i, dummy.catch, dummy.cv, locs )
      ...
      for( j in 1:length(rdi.report) ) {
        ...
        blah = dummy.cv$YEAR[ dummy.cv$YEAR %notin% dummy.catch$YEAR ]
        dummy.catch = bind_rows( dummy.catch, data.frame( YEAR = blah ) ) %>% arrange( YEAR )
...
convert.long.table.cat = function( cv.table, survey = c('MRIP','LACR','TPWD') ) {
  
  ###     ...where 'cv.table' is a table of recreational estimates ( catch, CV, & sample size ) which is
  ###               currently in wide-format and to be converted to long-format, and
  ...
  if( survey == 'MRIP' ) {
    ...
    dummy = cv.table %>%
      ...
      select( YEAR, NEW_MODEN, CATCH_VAR, METRIC, value ) %>%
      arrange( YEAR, NEW_MODEN, CATCH_VAR, METRIC )
    ...
    dummy = dummy %>%
      ...
      pivot_longer( !c(YEAR,NEW_MODEN,CATCH_VAR), names_to = 'METRIC', values_to = 'value' ) %>%
      filter( !is.na(value) ) %>%
    ...
    # dummy = tpwd.dummy %>%
      ...
      pivot_longer( !c(YEAR,NEW_MODEN), names_to = 'VARIABLE', values_to = 'value' ) %>%
